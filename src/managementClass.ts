...
export class StudentManagement {
  ...
  addStudent(name: string) {
    ...
    console.log(
      chalk.bgYellow.red(
        `Student : ${newStudent.name}, added successfully Student ID is : ${newStudent.id} `
      )
    );
    console.log(chalk.white("-").repeat(60));
  }
  ...
  enrollCourse(studentId: number, course: string) {
    ...
    if (student) {
      ...
      console.log(chalk.white("-").repeat(60));
      console.log(
        chalk.bgRed.white(`Student not found Please Enter a Valid ID`)
      );
      console.log(chalk.white("-").repeat(60));
    }
  }
  ...
  payStudentFees(studentId: number, course: string) {
    const student = this.findStudent(studentId);
    if (student) {
