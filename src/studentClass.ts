...
// Define a Course interface to keep track of course details
interface Course {
  name: string;
  fees: number;
  paid: boolean;
...
export class Student {
  ...
  viewBalance() {
    ...
    console.log(
      chalk.bgYellow.red(
        `Mr. ${this.name}, Your Remaining Balance is ${this.balalnce}`
      )
    );
    console.log(chalk.white("-").repeat(60));
  }
  ...
  payTutionFees(course: string) {
    ...
    if (enrolledCourses && !enrolledCourses.paid) {
      ...
      console.log(
        chalk.bgYellow.red(
          `You hvae successfully paid ${enrolledCourses.fees} for ${course}`
        )
      );
      console.log(chalk.white("-").repeat(60));
      ...
      console.log(chalk.white("-").repeat(60));
      console.log(
        chalk.bgYellow.red(`The Tution fee for ${course} has already been paid`)
      );
      console.log(chalk.white("-").repeat(60));
    } else {
      console.log(chalk.white("-").repeat(60));
      console.log(
        chalk.bgRed.white(`Your are not enrolled in this course ${course}`)
      );
      console.log(chalk.white("-").repeat(60));
    }
  }
  ...
  showStudentStatus() {
    ...
    this.courses.forEach((course) => {
      console.log(
        chalk.bgWhite.red(
          `* Name : ${course.name}, Fee: ${course.fees} Paid: ${course.paid}`
        )
      );
    });
